  ...
  Future<void> _deleteCoupon(String id) async {
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('Are you sure you want to delete this coupon? If it has been used, it will be deactivated instead.', style: GoogleFonts.inter(fontSize: 14, color: AppColors.textSecondary)),
        actions: [
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(res['message'] ?? 'Deleted successfully', style: const TextStyle(color: Colors.white)),
            ...
            actions: [
              IconButton(
                ...
                onPressed: () async {
                  ...
                  final res = await Navigator.push(context, MaterialPageRoute(builder: (_) => const AddCouponScreen()));
                ...
                children: [
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          suffixIcon: _searchController.text.isNotEmpty
                              ? IconButton(icon: const Icon(LucideIcons.x, size: 16), onPressed: () { _searchController.clear(); _loadCoupons(); })
                              : null,
                  ...
                  GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: _activeOnly ? AppColors.primaryDark : AppColors.surface,
                        borderRadius: BorderRadius.circular(16),
                        border: Border.all(color: _activeOnly ? AppColors.primaryDark : AppColors.cardBorder),
                      ),
                      child: Text('Active Only', style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w600, color: _activeOnly ? Colors.white : AppColors.textSecondary)),
                    ),
                  ),
                ],
              ),
            ...
            SliverFillRemaining(
              child: Center(
                child: Column(
                  ...
                  children: [
                    Icon(LucideIcons.ticket, size: 48, color: AppColors.textMuted.withValues(alpha: 0.5)),
                    const SizedBox(height: 16),
                    Text('No Coupons Found', style: GoogleFonts.playfairDisplay(fontSize: 20, fontWeight: FontWeight.w600, color: AppColors.textSecondary)),
                  ],
                ),
              ),
            )
  ...
  Widget _buildCouponCard(Map<String, dynamic> coupon) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: isActive ? AppColors.cardBorder : AppColors.cardBorder.withValues(alpha: 0.5)),
        boxShadow: [
            ...
            child: Row(
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: isActive ? AppColors.accent.withValues(alpha: 0.1) : AppColors.textMuted.withValues(alpha: 0.1),
                    borderRadius: BorderRadius.circular(12),
                    border: Border.all(color: isActive ? AppColors.accent.withValues(alpha: 0.3) : Colors.transparent),
                  ),
                  child: Column(
                    children: [
                      Text(valStr, style: GoogleFonts.inter(fontSize: 16, fontWeight: FontWeight.w800, color: isActive ? AppColors.accent : AppColors.textMuted)),
                        ...
                        children: [
                          ...
                          if (isActive)
                            Container(
                              ...
                              child: Text('Active', style: GoogleFonts.inter(fontSize: 10, fontWeight: FontWeight.w600, color: AppColors.success)),
                            )
                          else
                            Container(
                              padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                              decoration: BoxDecoration(color: AppColors.textMuted.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(4)),
                              child: Text('Inactive', style: GoogleFonts.inter(fontSize: 10, fontWeight: FontWeight.w600, color: AppColors.textMuted)),
                            ),
                        ],
                      ),
                ...
                PopupMenuButton<String>(
                  ...
                  onSelected: (v) {
                    ...
                    if (v == 'edit') {
                      Navigator.push(context, MaterialPageRoute(builder: (_) => AddCouponScreen(coupon: coupon))).then((r) { if (r == true) _loadCoupons(); });
                    }
