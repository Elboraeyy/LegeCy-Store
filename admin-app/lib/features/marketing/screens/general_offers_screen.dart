...
class _GeneralOffersScreenState extends State<GeneralOffersScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final data = await client.get('/api/admin/auth/offers');
  ...
  Future<void> _toggleActive(String id, bool isActive) async {
    ...
    try {
      final token = context.read<AuthProvider>().token;
      final client = ApiClient(token: token);
      await client.put('/api/admin/auth/offers/$id', body: { 'isActive': !isActive });
      _load();
    } catch (e) {
      if (mounted) setState(() => _isLoading = false);
  ...
  Future<void> _deleteOffer(String id) async {
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('Are you sure you want to delete this offer?', style: GoogleFonts.inter(fontSize: 14, color: AppColors.textSecondary)),
        actions: [
    ...
    try {
      final token = context.read<AuthProvider>().token;
      final client = ApiClient(token: token);
      await client.delete('/api/admin/auth/offers/$id');
      _load();
    } catch (e) {
      if (mounted) setState(() => _isLoading = false);
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error: $e')));
  ...
  void _showCreateDialog() {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(24)),
        title: Text('New Offer', style: GoogleFonts.playfairDisplay(fontWeight: FontWeight.w700, color: AppColors.primaryDark)),
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              try {
                final token = context.read<AuthProvider>().token;
                final client = ApiClient(token: token);
                await client.post('/api/admin/auth/offers', body: {
                  'name': nameController.text,
                  'offerType': 'GLOBAL',
                  'discountType': 'PERCENTAGE', // Assuming percentage for simplicity, could be enhanced
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Row(
          children: [
            const Icon(LucideIcons.tag, color: Color(0xFF10B981)),
            const SizedBox(width: 8),
            Text('General Offers', style: GoogleFonts.playfairDisplay(fontSize: 24, fontWeight: FontWeight.w700, color: AppColors.primaryDark)),
          ],
        ),
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          border: Border.all(color: isActive ? const Color(0xFF10B981).withValues(alpha: 0.5) : AppColors.cardBorder),
                          boxShadow: [
                            BoxShadow(
                          ...
                          children: [
                            Row(
                              ...
                              children: [
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Container(
                                        ...
                                        child: Text(
                                          o['discountType'] == 'PERCENTAGE' ? '${o['discountValue']}% OFF' : 'EGP ${o['discountValue']} OFF',
                                ...
                                Switch(
                                  value: isActive,
                                  onChanged: (v) => _toggleActive(o['id'], isActive),
                            ...
                            Row(
                              ...
                              children: [
                                Row(
                                  children: [
                                    Icon(LucideIcons.listFilter, size: 14, color: AppColors.textMuted),
                                    const SizedBox(width: 4),
                                    Text('Type: ${o['offerType'] ?? 'GLOBAL'}', style: GoogleFonts.inter(fontSize: 13, color: AppColors.textSecondary)),
                                  ],
                                ),
                                if (o['endDate'] != null)
                                  Row(
                                    children: [
                                      ...
                                      const SizedBox(width: 4),
                                      Text('Ends: ${_formatDate(o['endDate'])}', style: GoogleFonts.inter(fontSize: 13, color: AppColors.textSecondary)),
                                    ],
                                  ),
                              ],
                            ),
