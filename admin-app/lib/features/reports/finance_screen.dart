...
class _FinanceScreenState extends State<FinanceScreen> {
          ...
          slivers: [
            SliverAppBar(
              ...
              flexibleSpace: FlexibleSpaceBar(
                ...
                title: Text('Finance', style: GoogleFonts.playfairDisplay(fontSize: 22, fontWeight: FontWeight.w700, color: AppColors.primaryDark)),
              ),
              actions: [
                IconButton(
  ...
  List<Widget> _buildContent() {
    ...
    return [
      ...
      Row(children: [
        Expanded(child: _kpiCard('Revenue', _fmt(o['totalRevenue']), LucideIcons.trendingUp, AppColors.success)),
        const SizedBox(width: 10),
        Expanded(child: _kpiCard('Expenses', _fmt(o['totalExpenses']), LucideIcons.trendingDown, AppColors.error)),
      ]),
  ...
  Widget _buildProfitHero(Map<String, dynamic> o) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: (isPositive ? AppColors.primaryDark : AppColors.error).withValues(alpha: 0.3), blurRadius: 20, offset: const Offset(0, 10))],
      ),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Container(
            ...
            child: Icon(isPositive ? LucideIcons.trendingUp : LucideIcons.trendingDown, color: const Color(0xFF12403C), size: 22),
          ),
          const Spacer(),
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
            decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(10)),
            child: Text('$margin% margin', style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w700, color: Colors.white)),
          ),
        ]),
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('$count Pending Expenses', style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w700, color: AppColors.warning)),
          Text('${_fmt(total)} EGP awaiting approval', style: GoogleFonts.inter(fontSize: 11, color: AppColors.textMuted)),
        ])),
        Icon(LucideIcons.alertTriangle, size: 18, color: AppColors.warning.withValues(alpha: 0.6)),
      ]),
    );
  }

  // ── Monthly Comparison ──
  Widget _buildMonthlyComparison(Map<String, dynamic> thisM, Map<String, dynamic> lastM, Map<String, dynamic> growth) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _sectionHeader('MONTHLY P&L', LucideIcons.calendar, const Color(0xFF6366F1)),
        const SizedBox(height: 16),
        Row(children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            color: (revGrowth >= 0 ? AppColors.success : AppColors.error).withValues(alpha: 0.08),
            borderRadius: BorderRadius.circular(12),
          ),
          child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            Icon(revGrowth >= 0 ? LucideIcons.trendingUp : LucideIcons.trendingDown, size: 16,
                color: revGrowth >= 0 ? AppColors.success : AppColors.error),
            const SizedBox(width: 8),
            Text('${revGrowth.abs().toStringAsFixed(1)}% revenue ${revGrowth >= 0 ? 'growth' : 'decline'}',
  ...
  Widget _monthCol(String label, Map<String, dynamic> data) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(label, style: GoogleFonts.inter(fontSize: 10, fontWeight: FontWeight.w600, color: AppColors.textMuted, letterSpacing: 0.5)),
        ...
        _miniRow('Profit', _fmt(profit), profit >= 0 ? AppColors.primaryDark : AppColors.error),
      ]),
    );
  }
  ...
  Widget _buildRevenueChart(List<dynamic> trend) {
    final maxY = trend.fold<double>(0, (m, d) => max(m, (d['revenue'] as num).toDouble()));
    final spots = trend.asMap().entries.map((e) => FlSpot(e.key.toDouble(), (e.value['revenue'] as num).toDouble())).toList();

    return Container(
      padding: const EdgeInsets.fromLTRB(16, 20, 16, 16),
      decoration: BoxDecoration(color: AppColors.surface, borderRadius: BorderRadius.circular(20), border: Border.all(color: AppColors.cardBorder)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _sectionHeader('MONTHLY REVENUE', LucideIcons.barChart3, AppColors.success),
          ...
          child: BarChart(BarChartData(
            ...
            titlesData: FlTitlesData(
              leftTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, reservedSize: 44,
                  getTitlesWidget: (v, _) => Text(_fmtK(v), style: GoogleFonts.inter(fontSize: 9, color: AppColors.textMuted)))),
              bottomTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, reservedSize: 22,
                  getTitlesWidget: (v, _) {
                    final i = v.toInt();
                    if (i < 0 || i >= trend.length) return const SizedBox.shrink();
                    final month = (trend[i]['month'] as String).substring(5);
                    return Text(month, style: GoogleFonts.inter(fontSize: 8, color: AppColors.textMuted));
                  })),
            ...
            barGroups: spots.map((s) => BarChartGroupData(x: s.x.toInt(), barRods: [
              BarChartRodData(toY: s.y, width: 14, color: AppColors.success.withValues(alpha: 0.8),
  ...
  Widget _buildPaymentBreakdown(List<dynamic> payments) {
    final total = payments.fold<double>(0, (s, p) => s + ((p['revenue'] as num?)?.toDouble() ?? 0));
    final colors = [const Color(0xFF10B981), const Color(0xFF0EA5E9), const Color(0xFF8B5CF6), const Color(0xFFF59E0B), AppColors.error];
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                Row(children: [
                  ...
                  const SizedBox(width: 8),
                  Text((p['method'] ?? 'Unknown').toString().toUpperCase(), style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w600)),
                ]),
                Text('${_fmt(rev)} EGP', style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w700, color: color)),
              ]),
              const SizedBox(height: 4),
              Row(children: [
                Expanded(child: ClipRRect(
                  borderRadius: BorderRadius.circular(3),
                  child: LinearProgressIndicator(value: pct / 100, minHeight: 5, backgroundColor: AppColors.cardBorder, color: color),
                )),
                const SizedBox(width: 10),
                Text('${pct.toStringAsFixed(0)}% · $orders orders', style: GoogleFonts.inter(fontSize: 9, color: AppColors.textMuted)),
              ]),
            ]),
          );
        }),
  ...
  Widget _buildExpenseCategories(List<dynamic> categories) {
    final totalExp = categories.fold<double>(0, (s, c) => s + ((c['amount'] as num?)?.toDouble() ?? 0));
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(color: AppColors.surface, borderRadius: BorderRadius.circular(20), border: Border.all(color: AppColors.cardBorder)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _sectionHeader('EXPENSE CATEGORIES', LucideIcons.tag, AppColors.error),
        const SizedBox(height: 16),
        ...categories.map((c) {
          ...
          return Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Expanded(child: Text(c['category'] ?? 'Unknown', style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w600))),
                if (overBudget) ...[
                  Icon(LucideIcons.alertTriangle, size: 12, color: AppColors.error),
                  const SizedBox(width: 4),
                ],
                Text('${_fmt(amount)} EGP', style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w700, color: overBudget ? AppColors.error : AppColors.textPrimary)),
              ]),
              const SizedBox(height: 4),
              Row(children: [
                Expanded(child: ClipRRect(
                  borderRadius: BorderRadius.circular(3),
                  child: LinearProgressIndicator(value: pct / 100, minHeight: 5, backgroundColor: AppColors.cardBorder, color: overBudget ? AppColors.error : AppColors.warning),
                )),
                const SizedBox(width: 10),
                Text('${pct.toStringAsFixed(0)}% · $count items', style: GoogleFonts.inter(fontSize: 9, color: AppColors.textMuted)),
              ]),
              if (budget != null) ...[
                const SizedBox(height: 2),
                Text('Budget: ${_fmt(budget)} EGP', style: GoogleFonts.inter(fontSize: 9, color: overBudget ? AppColors.error : AppColors.textMuted)),
              ],
            ]),
          );
        }),
  ...
  Widget _buildExpenseCard(dynamic expense) {
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(expense['title'] ?? 'Expense', style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w600), overflow: TextOverflow.ellipsis),
          const SizedBox(height: 2),
          Row(children: [
            Text(expense['category'] ?? '', style: GoogleFonts.inter(fontSize: 11, color: AppColors.textMuted)),
            if (expense['paidBy'] != null) ...[
              Text(' · ', style: GoogleFonts.inter(fontSize: 11, color: AppColors.textMuted)),
              Text(expense['paidBy'], style: GoogleFonts.inter(fontSize: 11, color: AppColors.textMuted)),
            ],
          ]),
        ])),
        Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
          Text('${_fmt(expense['amount'])} EGP', style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w700, color: AppColors.error)),
          const SizedBox(height: 4),
          Row(mainAxisSize: MainAxisSize.min, children: [
            ...
            const SizedBox(width: 4),
            Text(dateStr, style: GoogleFonts.inter(fontSize: 9, color: AppColors.textMuted)),
          ]),
        ]),
      ]),
    );
