...
class _PartnersScreenState extends State<PartnersScreen> {
          ...
          slivers: [
            SliverAppBar(
              ...
              flexibleSpace: FlexibleSpaceBar(
                ...
                title: Text('Partners', style: GoogleFonts.playfairDisplay(fontSize: 22, fontWeight: FontWeight.w700, color: AppColors.primaryDark)),
              ),
              actions: [
                IconButton(
                ...
                sliver: SliverList(delegate: SliverChildListDelegate([
                  ...
                  for (int i = 0; i < 2; i++) ...[
                    Container(
                      ...
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Row(children: [
                          const AppShimmer(width: 44, height: 44, borderRadius: 14),
                          const SizedBox(width: 14),
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            Row(children: const [
                              AppShimmer(width: 100, height: 15),
                              SizedBox(width: 6),
                              AppShimmer(width: 50, height: 16, borderRadius: 6),
                            ]),
  ...
  List<Widget> _buildContent() {
    ...
    return [
      ...
      // ── KPIs ──
      Row(children: [
        Expanded(child: _kpi('Capital', _fmt(o['totalCapital']), LucideIcons.landmark, const Color(0xFF6366F1))),
        const SizedBox(width: 10),
        Expanded(child: _kpi('Net Profit', _fmt(o['netProfit']), LucideIcons.trendingUp, AppColors.success)),
      ]),
      const SizedBox(height: 10),
      Row(children: [
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Business Overview', style: GoogleFonts.inter(fontSize: 12, color: Colors.white60)),
            Text('${_fmt(o['totalRevenue'])} EGP Revenue', style: GoogleFonts.inter(fontSize: 18, fontWeight: FontWeight.w700, color: Colors.white)),
          ]),
        ]),
        const SizedBox(height: 16),
  ...
  Widget _heroM(String l, String v) => Expanded(child: Column(children: [
    Text(v, style: GoogleFonts.inter(fontSize: 15, fontWeight: FontWeight.w700, color: Colors.white)),
  ...
  Widget _buildInvestorCard(dynamic inv) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: isActive ? AppColors.cardBorder : AppColors.error.withValues(alpha: 0.3)),
      ),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        // Header
        Row(children: [
          Container(
            width: 44, height: 44,
            decoration: BoxDecoration(
              gradient: LinearGradient(colors: [AppColors.primaryDark, AppColors.primaryDark.withValues(alpha: 0.7)]),
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Flexible(child: Text(inv['name'] ?? '', style: GoogleFonts.inter(fontSize: 15, fontWeight: FontWeight.w700), overflow: TextOverflow.ellipsis)),
              ...
              Container(
                ...
                child: Text(isActive ? 'ACTIVE' : 'INACTIVE', style: GoogleFonts.inter(fontSize: 8, fontWeight: FontWeight.w700, color: isActive ? AppColors.success : AppColors.error)),
              ),
            ]),
            Text('${(inv['type'] ?? 'PARTNER').toString()} · Share: ${(share * 100).toStringAsFixed(1)}%', style: GoogleFonts.inter(fontSize: 11, color: AppColors.textMuted)),
          ])),
        ]),
        const SizedBox(height: 14),
        ...
        Container(
          ...
          child: Row(children: [
            ...
            Container(width: 1, height: 36, color: AppColors.cardBorder),
            _finCol('Net', _fmt(net), net >= 0 ? AppColors.primaryDark : AppColors.error),
          ]),
        ),
        ...
        if (recentTx.isNotEmpty) ...[
          const SizedBox(height: 12),
          Text('Recent Transactions', style: GoogleFonts.inter(fontSize: 10, fontWeight: FontWeight.w600, color: AppColors.textMuted)),
          const SizedBox(height: 6),
  ...
  Widget _buildPartnerCard(dynamic p) {
    ...
    final growth = lastMonth > 0 ? ((thisMonth - lastMonth) / lastMonth * 100) : 0.0;
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          Container(
            ...
            child: Center(child: Text(
              ...
              style: GoogleFonts.playfairDisplay(fontSize: 20, fontWeight: FontWeight.w700, color: AppColors.accent),
            )),
          ),
          const SizedBox(width: 14),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Flexible(child: Text(p['name'] ?? '', style: GoogleFonts.inter(fontSize: 15, fontWeight: FontWeight.w700), overflow: TextOverflow.ellipsis)),
              const SizedBox(width: 6),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                decoration: BoxDecoration(color: AppColors.accent.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(6)),
                child: Text('${(rate * 100).toStringAsFixed(0)}%', style: GoogleFonts.inter(fontSize: 9, fontWeight: FontWeight.w700, color: AppColors.accent)),
              ),
            ]),
            Text('Code: ${p['code'] ?? '-'} · ${p['orderCount'] ?? 0} orders', style: GoogleFonts.inter(fontSize: 11, color: AppColors.textMuted)),
          ])),
        ]),
        const SizedBox(height: 14),
        ...
        Container(
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('Wallet Balance', style: GoogleFonts.inter(fontSize: 10, color: AppColors.textMuted)),
              Text('${_fmt(wallet)} EGP', style: GoogleFonts.inter(fontSize: 18, fontWeight: FontWeight.w700, color: AppColors.primaryDark)),
            ]),
            if (growth != 0)
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                decoration: BoxDecoration(color: (growth >= 0 ? AppColors.success : AppColors.error).withValues(alpha: 0.1), borderRadius: BorderRadius.circular(8)),
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  Icon(growth >= 0 ? LucideIcons.trendingUp : LucideIcons.trendingDown, size: 12, color: growth >= 0 ? AppColors.success : AppColors.error),
                  const SizedBox(width: 4),
                  Text('${growth.abs().toStringAsFixed(0)}%', style: GoogleFonts.inter(fontSize: 10, fontWeight: FontWeight.w700, color: growth >= 0 ? AppColors.success : AppColors.error)),
                ]),
              ),
          ]),
        ),
        ...
        Container(
          ...
          child: Column(children: [
            Row(children: [
              _finCol('Commissions', _fmt(commissions), AppColors.success),
              Container(width: 1, height: 36, color: AppColors.cardBorder),
              _finCol('Payouts', _fmt(payouts), AppColors.error),
              Container(width: 1, height: 36, color: AppColors.cardBorder),
              _finCol('Pending', _fmt(commissions - payouts), const Color(0xFFF59E0B)),
            ]),
            const SizedBox(height: 10),
            Container(height: 1, color: AppColors.cardBorder),
            const SizedBox(height: 10),
  ...
  Widget _txRow(dynamic tx, {required bool isInvestor}) {
    ...
    return Padding(
      ...
      child: Row(children: [
        Container(
          width: 6, height: 6,
          decoration: BoxDecoration(color: isCredit ? AppColors.success : AppColors.error, shape: BoxShape.circle),
        ),
        const SizedBox(width: 8),
        Expanded(child: Text(type, style: GoogleFonts.inter(fontSize: 11, fontWeight: FontWeight.w500, color: AppColors.textSecondary))),
        Text('${isCredit ? '+' : '-'}${_fmt(amount)}', style: GoogleFonts.inter(fontSize: 11, fontWeight: FontWeight.w700, color: isCredit ? AppColors.success : AppColors.error)),
        const SizedBox(width: 8),
  ...
  Widget _alertCard(dynamic alert) {
    ...
    return Container(
      ...
      child: Row(children: [
        Icon(severity == 'critical' ? LucideIcons.alertOctagon : LucideIcons.alertTriangle, size: 18, color: color),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(alert['title'] ?? '', style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w600, color: color)),
          Text(alert['message'] ?? '', style: GoogleFonts.inter(fontSize: 10, color: AppColors.textMuted), maxLines: 2, overflow: TextOverflow.ellipsis),
        ])),
      ]),
    );
  }
  ...
  // ── Helpers ──
  Widget _finCol(String l, String v, Color c) => Expanded(child: Column(children: [
  ...
  Widget _miniStat(String l, String v) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Text(l, style: GoogleFonts.inter(fontSize: 9, color: AppColors.textMuted)),
    Text(v, style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w600)),
  ]);
