...
class _BrandsScreenState extends State<BrandsScreen> {
  ...
  Future<void> _loadBrands() async {
    ...
    try {
      ...
      if (mounted) setState(() { _brands = data['brands'] ?? []; _isLoading = false; });
    } catch (e) {
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Container(
        ...
        child: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          _field('Brand Name (EN)', nameCtrl, LucideIcons.tag),
          const SizedBox(height: 12),
          _field('Brand Name (AR)', nameArCtrl, LucideIcons.languages),
          const SizedBox(height: 12),
          Focus(
          ...
          : _error != null
              ...
              : _brands.isEmpty
                  ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                      Container(padding: const EdgeInsets.all(24), decoration: BoxDecoration(color: const Color(0xFF8B5CF6).withValues(alpha: 0.1), shape: BoxShape.circle), child: const Icon(LucideIcons.tag, size: 48, color: Color(0xFF8B5CF6))),
                      const SizedBox(height: 24),
                      Text('No Brands Yet', style: GoogleFonts.playfairDisplay(fontSize: 22, fontWeight: FontWeight.w700, color: AppColors.primaryDark)),
                      const SizedBox(height: 8),
                      Text('Add brands to organize your catalog.', style: GoogleFonts.inter(fontSize: 14, color: AppColors.textMuted)),
                    ]))
                  : RefreshIndicator(
