    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold">{t.orders.fraud.title || 'Fraud Review Queue'}</h1>
                    <p className="text-gray-500 mt-1">
                        {t.orders.fraud.subtitle || 'Review and approve/reject orders flagged for potential fraud'}
                    </p>
                </div>
                <div className="flex items-center gap-2">
                    <span className="px-3 py-1 bg-amber-100 text-amber-700 rounded-full text-sm font-medium">
                        {reviews.length} {t.orders.returns.tabs?.pending || 'Pending'}
                    </span>
                </div>
            </div>
            ...
            {reviews.length === 0 ? (
                <div className="bg-white rounded-lg shadow-sm border p-12 text-center">
                    ...
                    <h3 className="text-xl font-semibold text-gray-700">{t.orders.fraud.all_clear}</h3>
                    <p className="text-gray-500 mt-2">{t.orders.fraud.no_pending}</p>
                </div>
            ) : (
                <div className="space-y-4">
                    {reviews.map((review) => (
                        <div key={review.id} className="bg-white rounded-lg shadow-sm border overflow-hidden">
                            ...
                            <div className="p-4 flex items-center justify-between border-b">
                                <div className="flex items-center gap-4">
                                    <span className={`px-3 py-1 rounded-full text-sm font-bold border ${getRiskColor(Number(review.score))}`}>
                                        {t.orders.fraud.risk_score}: {Number(review.score)}
                                    </span>
                                    <div>
                                        <p className="font-medium">{t.orders.table.order} #{review.orderId.slice(0, 8)}</p>
                                        <p className="text-sm text-gray-500">
                                            {review.order.customerName || (t.orders.details.guest || 'Unknown')} • {review.order.customerEmail}
                                        </p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-4">
                                    ...
                                    >
                                        {expandedId === review.id ? (t.orders.failed_payments.hide || 'Less') : (t.orders.failed_payments.details || 'More')}
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="p-4 bg-gray-50">
                                <p className="text-sm font-medium text-gray-600 mb-2">{t.orders.fraud.risk_factors}:</p>
                            ...
                            {expandedId === review.id && (
                                <div className="p-4 border-t">
                                    <h4 className="font-medium mb-2">{t.orders.fraud.order_items}:</h4>
                                    ...
                                    <div className="mt-4 text-sm text-gray-500">
                                        <p>{t.common.phone}: {review.order.customerPhone || 'N/A'}</p>
                                        <p>{t.orders.table.date}: {new Date(review.order.createdAt).toLocaleString(language === 'ar' ? 'ar-EG' : 'en-US')}</p>
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="p-4 border-t flex justify-end gap-2">
                                ...
                                >
                                    {processingId === review.orderId ? t.orders.fraud.processing : ('✓ ' + t.orders.fraud.approve)}
                                </button>
                                <button
                                    onClick={() => {
