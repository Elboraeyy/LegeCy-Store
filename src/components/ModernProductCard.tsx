...
export default function ModernProductCard({
  ...
  return (
    <>
      <div
        className={`group modern-card relative w-full bg-white rounded-lg overflow-hidden border border-gray-100/50 shadow-sm cursor-pointer ${isOutOfStock ? "opacity-60 grayscale-[20%]" : ""}`}
      >
        {/* 1. Image Container */}
            ...
            {!isOutOfStock &&
              ...
              product.detailTags.map((tag, idx) => {
                ...
                return (
                  <span
                    key={idx}
                    className="px-2.5 py-1 text-[10px] font-extrabold text-white rounded-full tracking-tight uppercase shadow-sm"
