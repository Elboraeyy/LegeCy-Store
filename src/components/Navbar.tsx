                    ...
                    <nav className="flex items-center justify-between">
                        ...
                        <div className="hidden lg:flex items-center gap-8 ml-8">
                             ...
                             >
                                ...
                                >
                                    Shop
                                    <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="opacity-60 group-hover:rotate-180 transition-transform duration-200">
                                        <path d="M6 9l6 6 6-6"/>
                                    </svg>
                                </NavLink>
                                ...
                                <AnimatePresence>
                                    {showCategoryMenu && (
                                        ...
                                        >
                                            <Link href="/shop" className="block px-6 py-3 text-sm text-[#12403C] hover:bg-[#F5F0E3] hover:text-[#d4af37] transition-colors font-medium">
                                                All Products
                                            </Link>
                                            ...
                                            {categories.length > 0 ? (
                                                ...
                                                <span className="block px-6 py-3 text-sm text-gray-400">No categories found</span>
                                            )}
                                        </motion.div>
                                    )}
                                </AnimatePresence>
                        ...
                        <div className="flex items-center gap-4 lg:gap-6">
                            ...
                            {(headerSettings?.showWishlist ?? true) && (
                                <Link href="/wishlist" className="relative p-2 text-[#12403C] hover:text-[#d4af37] transition-colors group">
                                    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="group-hover:fill-current/10 transition-colors">
                             ...
                             {isClient && (headerSettings?.showAccount ?? true) && (
                                <div className="hidden lg:block relative">
                                    {user ? (
                                                ...
                                                {showUserMenu && (
                                                    ...
                                                    >
                                                        <div className="px-4 py-3 bg-gray-50/50 rounded-xl mb-1">
                                                            <p className="text-sm font-semibold text-[#12403C] truncate">{user.name || 'User'}</p>
                                                            <p className="text-xs text-gray-500 truncate">{user.email}</p>
                                                        </div>
                                                        
                                                        <div className="space-y-px">
                                                            <Link href="/account" className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-[#F5F0E3] hover:text-[#12403C] rounded-lg transition-colors" onClick={() => setShowUserMenu(false)}>
                                                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                                                                My Account
                                                            </Link>
                                                            <Link href="/account/orders" className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-[#F5F0E3] hover:text-[#12403C] rounded-lg transition-colors" onClick={() => setShowUserMenu(false)}>
                                        ...
                                        <Link href="/login" className="flex items-center gap-2 px-5 py-2.5 bg-[#12403C] text-white rounded-full text-sm font-medium hover:bg-[#0e3330] hover:shadow-lg hover:shadow-[#12403C]/20 transition-all transform hover:-translate-y-0.5">
                                            Sign In
                                        </Link>
                                    )}
                                </div>
                ...
                {isOpen && (
                    <>
                        ...
                        <motion.div
                            ...
                            className="fixed top-0 right-0 bottom-0 w-full max-w-sm bg-white z-[70] shadow-2xl p-6 flex flex-col"
                        >
                            <div className="flex items-center justify-between mb-8">
                                <span className="text-xl font-bold font-heading text-[#12403C] uppercase">Menu</span>
                                <button onClick={() => setIsOpen(false)} className="p-2 text-gray-400 hover:text-gray-800 bg-gray-50 rounded-full">
                            ...
                            <div className="flex-1 overflow-y-auto space-y-6">
                                <nav className="flex flex-col space-y-4">
                                    ...
                                    <MobileNavLink href="/help" onClick={() => setIsOpen(false)}>Help</MobileNavLink>
                                </nav>
                                    ...
                                    {user ? (
                                        <div className="space-y-4">
                                            <div className="flex items-center gap-3 mb-4">
                                                ...
                                                <div>
                                                    <p className="font-semibold text-[#12403C]">{user.name}</p>
                                                    <p className="text-xs text-gray-500">{user.email}</p>
                                                </div>
                                            </div>
                                            <MobileNavLink href="/account" onClick={() => setIsOpen(false)} icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>}>
                                                My Account
                                            </MobileNavLink>
                                            <MobileNavLink href="/account/orders" onClick={() => setIsOpen(false)} icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18"/><path d="M9 21V9"/></svg>}>
                                                My Orders
                                            </MobileNavLink>
                                            <button 
...
function NavLink({ href, active, children, className = "" }: { href: string; active?: boolean; children: React.ReactNode; className?: string }) {
    return (
        ...
        >
            {children}
            <span className={`absolute bottom-0 left-0 h-[2px] bg-[#d4af37] transition-all duration-300 ${active ? "w-full" : "w-0 group-hover:w-full"}`}></span>
        </Link>
    );
}
