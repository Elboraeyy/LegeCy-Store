...
export async function placeOrderWithShipping(
  input: CheckoutInput,
  ...
  try {
    ...
    // KILL SWITCH CHECKS
    // ========================================
    const { getKillSwitches, isPaymentMethodEnabled } =
      await import("@/lib/killSwitches");
    const switches = await getKillSwitches();

    if (!switches.checkout_enabled) {
      return {
        success: false,
        error: "Checkout is temporarily disabled. Please try again later.",
      };
    }
    ...
    if (!(await isPaymentMethodEnabled(input.paymentMethod))) {
      return {
        success: false,
        error: `${input.paymentMethod === "cod" ? "Cash on delivery" : "Online payment"} is currently unavailable.`,
      };
    }
    ...
    if (input.couponCode && !switches.coupons_enabled) {
      return {
        success: false,
        error: "Coupon codes are temporarily disabled.",
      };
    }
    ...
    // RATE LIMITING CHECK
    // ========================================
    const { checkCheckoutRateLimit, getClientIdentifier } =
    ...
    if (input.idempotencyKey) {
      ...
      if (existingOrder) {
        logger.info(`Idempotent order detected, returning existing order`, {
    ...
    const variantIds = input.cartItems
    ...
    if (input.couponCode) {
      ...
      if (
        validation.isValid &&
        validation.coupon &&
        validation.finalTotal !== undefined
      ) {
    ...
    const pointsEarned = loyaltySettings.enabled
      ? Math.floor(finalTotal * loyaltySettings.pointsPerEgp)
      : 0;
    ...
    const customerIP =
      headersListForRateLimit.get("x-forwarded-for")?.split(",")[0]?.trim() ||
    ...
    if (fraudCheckPromise) {
      ...
      if (fraudAnalysis.shouldBlock) {
        ...
        return {
          success: false,
          error:
            "Your order cannot be processed with Cash on Delivery at this time. Please try a valid online payment method.",
        };
      }
    }
    ...
    const order = await prisma.$transaction(
      async (tx) => {
        ...
        const initialStatus =
          input.paymentMethod === "wallet" || input.paymentMethod === "instapay"
        ...
        const subtotalAmount = input.cartItems.reduce(
              ...
              create: input.cartItems.map((item) => {
                ...
                if (totalDiscountFromCoupon > 0 && subtotalBeforeCoupon > 0) {
                  const itemDiscountShare =
                    totalDiscountFromCoupon *
                    (itemTotal / subtotalBeforeCoupon);
                  discountedPricePerUnit =
                    item.price - itemDiscountShare / item.qty;
                ...
                const costAtPurchase = item.variantId
                  ? (variantCostMap[item.variantId] ?? null)
                  : null;
                ...
                return {
                  ...
                  name: item.name,
                  sku: item.variantId
                    ? (variantSkuMap[item.variantId] ?? null)
                    : null, // SKU snapshot
    ...
    (async () => {
      try {
        ...
        const itemsSubtotal = input.cartItems.reduce(
          (sum, item) => sum + item.price * item.qty,
          0,
        );
    ...
    try {
      ...
      logger.error("Failed to check inventory for notifications", {
        error: err,
      });
    }
    ...
    if (error instanceof Error) {
      ...
      if (msg.includes("insufficient stock") || msg.includes("available:")) {
        ...
        errorMessage =
          "Unable to process order at this time. Please contact support.";
